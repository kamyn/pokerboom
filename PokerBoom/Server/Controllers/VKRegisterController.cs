...
namespace PokerBoom.Server.Controllers
{
    ...
    [ApiController]
    public class VKRegisterController : ControllerBase
    {
        private readonly SignInManager<ApplicationUser> _signInManager;
        private readonly UserManager<ApplicationUser> _userManager;
        public VKRegisterController(UserManager<ApplicationUser> userManager,
        ...
        [HttpGet]
        [ActionName("vkregister")]
        public async Task<IActionResult> VKRegister(string code, string username)
        {
            if (await _userManager.FindByNameAsync(username) != null)
                return Ok(new LoginResultViewModel { Success = false, Error = "Пользователь с таким именем уже существует" });
            ...
            if (user == null)
            {
                var account = new ApplicationUser { UserName = username, EmailConfirmed = false, VkId = response.UserId.ToString(), Currency = 1000 };
            ...
            return BadRequest(new LoginResultViewModel { Success = false });
        }
    }
}
